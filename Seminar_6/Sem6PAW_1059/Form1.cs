...
namespace Sem6PAW_1059
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            int latimeTb = ((TextBox)listaTb[0]).Width;

            for (int i=0;i<4;i++)
            {
                TextBox tbNou = new TextBox();
                tbNou.Location = new Point(x, y + 30);
                tbNou.Width = latimeTb;
                x += dist;
                if (i == 3)
                    tbNou.ReadOnly = true;
                if(i==1 || i==2)
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < listaTb.Count; i+=4)
            {
                ...
                else
                {
                    ...
                    {
                        string denumire =
                            ((TextBox)listaTb[i]).Text;
                        float pret = 
                            (float)Convert.ToDouble(((TextBox)listaTb[i + 1]).Text);
                        float cantitate =
                            (float)Convert.ToDouble(((TextBox)listaTb[i + 2]).Text);
